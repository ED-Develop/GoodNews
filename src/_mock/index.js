...
mock.onGet('/login').reply((config) => {
    ...
    if (users.some( user => config.params.email === user.email && config.params.password === user.password)) {
        ...
        const {id, login, email, isSubscribe, region, membership} = users.find( user => user.email === config.params.email);
...
mock.onGet('/auth').reply((config) => {
    ...
    if (user) {
        const {id, login, email, isSubscribe, region, membership} = user;
